...
import com.michaelflisar.changelog.tags.ChangelogTagImage;
...
class ChangelogParserUtil {
    ...
    private static void readReleaseNode(XmlPullParser parser, Changelog changelog, IAutoVersionNameFormatter autoVersionNameFormatter) throws Exception {
        ...
        while (parser.next() != XmlPullParser.END_TAG) {
            ...
            if (changelogTag != null) {
                Log.d("TEST", "tag - " + changelogTag.getXMLTagName());
                if (changelogTag.getXMLTagName().equals(ChangelogTagImage.TAG)) {
                    readImageNode(changelogTag, parser, changelog, release);
                } else {
    ...
    private static void readImageNode(IChangelogTag changelogTag, XmlPullParser parser, Changelog changelog, ItemRelease release) throws Exception {
        Log.d("TEST", "readImageNode");
        if (parser == null || changelog == null) {
            return;
        }
