    ...
    public ChangelogRecyclerViewAdapter setupEmptyRecyclerView(RecyclerView recyclerView) {
        ...
        ((GridLayoutManager) recyclerView.getLayoutManager()).setSpanSizeLookup(new GridLayoutManager.SpanSizeLookup() {
            ...
            public int getSpanSize(int position) {
                if (adapter.getItemViewType(position) == ChangelogRecyclerViewAdapter.Type.Image.ordinal()) {
                    if (adapter.getItemViewType(position + 1) == ChangelogRecyclerViewAdapter.Type.Image.ordinal()
                        || adapter.getItemViewType(position - 1) == ChangelogRecyclerViewAdapter.Type.Image.ordinal()) {
                        return 1;
                    }
                }
                return 2;
